...
public class ControllerImpl implements Controller {
    ...
    private static final Set<String> SIMPLE_BUILDINGS = Set.of("depurator", "foundry", "house", "mine", "power plant",
            "woodcutter");
    ...
    public ControllerImpl(final City city) {
        ...
        Arrays.stream(Resource.values())
                .flatMap(res -> Stream.of(factory.createSimpleProductionBuilding(res),
                        factory.createAdvancedProductionBuilding(res)))
    ...
    // idTable = 0 means revenue, idTable = 1 means construction, idTable = 2 means
    // upgrade.
    private Map<Resource, Integer> getTable(final String name, final int idTable) {
        final Optional<ProductionBuilding> building = this.allBuildings.stream().filter(b -> b.getName().equals(name))
                .findFirst();
        if (building.isEmpty()) {
