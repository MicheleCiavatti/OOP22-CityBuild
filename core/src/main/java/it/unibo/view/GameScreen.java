...
public class GameScreen extends ScreenAdapter {
    ...
    // The game is structured in cycles: at the end of a cycle, it checks for new
    // citizens and updates the resources
    ...
    private final String[] buildingList = { "Item1", "Item2", "Item3", "House", "Lumber_refinary", "Mine",
            "Mineral_station", "Power_plant", "Quantum_reactor", "Skyscraper", "Ultrafiltration_complex",
            "Woodcutter" };
    ...
    private int index = 0;
    private final String[] imageList = { "Depuratoricon", "Forgeicon", "Foundryicon", "Houseicon",
            "Lumber_refinaryicon", "Mineicon",
            "Mineral_stationicon", "Power_planticon", "Quantum_reactoricon", "Skyscrapericon",
            "Ultrafiltration_complexicon", "Woodcuttericon" };
    ...
    public GameScreen(final Controller controller) {
        ...
        // Setting up the tablePlayer that contains the resources in possesion of the
        // player
    ...
    /*
     * This method is called at the end of every cycle of the game to update the
     * resources and citizens in town.
     * It is also called when the player spends resources to create or upgrade
     * buildings.
     */
    private void updateTablePlayer() {
        ...
        this.labelResources.setText(this.computeTextResources(this.resources) +
                Resource.CITIZEN + ": " + this.controller.getCitizensInTown() + "/"
                + this.resources.get(Resource.CITIZEN));
    }

    // Given a map of resources, returns a string containing the resources with the
    // respective value (except for CITIZEN)
    ...
    /*
     * A position is not valid when overlaps with another building or when out of
     * the screen.
     */
    ...
    private class GameProcessor extends InputAdapter {
        ...
        public boolean mouseMoved(final int screenX, final int screenY) {
            ...
            if (selected.isPresent()) { // Verifies if the user has selected a building to place
                ...
                return true;
            } else { // Allows to display a label containing info about upgrade costs when mouse is
                     // over buildings
        ...
        // Creates a square for placing the building and stores the name of the building
        // selected, retrieving it from the icon.
         ...
         * It allows to place the building selected only if not overlapping another
         * building and if the player
        ...
        private boolean handlePlacement() {
            if (isValidPosition(selected.get()) && controller.checkResourcesAndBuild(this.selectedName)) {
                ...
                final var im = new Image(new Texture(
                        Gdx.files.internal(IMAGE_FOLDER + imageList[index].replace(ICON_SUFFIX, EXTENSION))));
        ...
        /*
         * This method is used to determine the consequences of a click of the mouse
         * when not carrying a building for placement.
